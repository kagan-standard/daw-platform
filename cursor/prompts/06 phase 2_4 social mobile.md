...
|------|------|------------|-----------|
| 2025-02-15 | All | API already has users, stats, map/user, exchange/portfolio, beers/:name, ratings cheers; no server changes. | Phase forbids modifying server.js. |
| 2025-02-15 | Profile | Beer trail shown as list of locations (no Leaflet) when map view not in app. | No new CDN; Phase 2.3 map may add Leaflet later. |
| 2025-02-15 | Leaderboard | Most Cheered from client-side cheers cache. | Leaderboard API has no cheers category; not modifying API. |
| 2025-02-15 | PWA | Icons generated by user via generate-icons.html; icons/ holds README until then. | Cannot generate PNG server-side without extra deps. |
| 2025-02-15 | Shortcuts | M and E only work when view-map and view-exchange exist. | Phase 2.3 may add those views; navigate guards on missing views. |