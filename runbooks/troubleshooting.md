...
- **Check:** `docker ps` — if `beerbook-api` is restarting or missing, check `docker logs beerbook-api`. Look for `Cannot find module './routes/...'`. If the build log shows cached layers and **no** `COPY routes ./routes` step, the Dockerfile on the VPS may be old or the build used cache and skipped the new layer.
- **Fix:**  
  1. On the VPS, confirm `apps/beerbook-api/Dockerfile` contains the line `COPY routes ./routes`.  
  2. Rebuild with **no cache** so the routes layer is included:  
     `docker compose -f /opt/daw-platform/infra/compose/docker-compose.yml --env-file /opt/daw-platform/infra/compose/.env build --no-cache beerbook-api`  
     then `docker compose -f /opt/daw-platform/infra/compose/docker-compose.yml --env-file /opt/daw-platform/infra/compose/.env up -d beerbook-api`.
